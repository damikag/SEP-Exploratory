(function () {
  var id = document.location.pathname.substring(1);
  ...
  if (window.addEventListener) {
      ...
      function () {
        ...
        function init() {
          ...
          socket.on("on-connected", (onlineUsers) => {
            ...
            updateOnlineUsers(keys, onlineUsers);
          });

          socket.on("user-connected", (newUserData) => {
            console.log("new user", newUserData);
            addNewUser(newUserData);
          });

          function addNewUser(newUserData) {
            var tbodyElement = document.getElementById("tbody-id");
            const trElement = document.createElement("tr");
            trElement.setAttribute("id", newUserData.id);
            const tdElement_1 = document.createElement("td");
            const image = document.createElement("img");
            image.setAttribute("src", "images/online.jpg");
            //           image.setAttribute("src", "image/online.jpeg");
            image.setAttribute("width", "20");
            image.setAttribute("height", "20");
            // image.setAttribute("alt", "The Pulpit Rock");
            tdElement_1.appendChild(image);
            // tdElement_1.innerText = tbodyElement.childNodes.length;
            const tdElement_2 = document.createElement("td");
            tdElement_2.innerText = newUserData.name;
            const tdElement_3 = document.createElement("td");
            tdElement_3.innerText = newUserData.email;
          ...
          function updateOnlineUsers(keys, onlineUsers) {
            keys.map((key) => {
              addNewUser(onlineUsers[key]);
            });
          }

          function removeUser(user) {
            var trElement = document.getElementById(user.id);
            trElement.remove();
          }

          socket.on("user-disconnected", (removed_user) => {
            console.log("left users : ", removed_user);
            removeUser(removed_user);
          });
          ...
          function download() {
            ...
            link.href = document.getElementById("imageView").toDataURL();
            link.click();
          }
    ...
    $("#draw-grid").change(function(){
        ...
        function img_update(trans) {
          ...
          socket.emit("copyCanvas", roomName, {
            transferCanvas: true,
          });
        }
        ...
        function drawRect(min_x, min_y, abs_x, abs_y, color, linewidth, emit) {
          ...
          socket.emit("rectangle", roomName, {
            min_x: min_x / w,
            min_y: min_y / h,
            abs_x: abs_x / w,
            abs_y: abs_y / h,
        ...
        function old_drawCircle(x1, y1, x2, y2, color, linewidth, emit) {
          ...
          socket.emit("circledraw", roomName, {
            x1: x1 / w,
            y1: y1 / h,
            x2: x2 / w,
            y2: y2 / h,
        ...
        function drawEllipse(x, y, w, h, color, linewidth, emit) {
          ...
          socket.emit("ellipsedraw", roomName, {
            x: x,
            y: y,
            w: w,
            h: h,
